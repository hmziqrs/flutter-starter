...
- The [Implemented feature API contracts](#implemented-feature-api-contracts) section below
  freezes the **current** (implemented) feature public APIs — consult it before changing any
  feature's public surface. The `features/` specs are the *intake* list for planned features.
...
## Implemented feature API contracts

The frozen public API boundaries of the implemented baseline features (the freeze layer — consult before changing any feature public surface): typed values, page callbacks, and the root integration sequence.
