...
- **Partly present.** This is a deepening of an existing, tested file — not a new subsystem. The infrastructure subsystem in [`architecture.md`](../../../docs/architecture.md) establishes `AppLogger` (which consumes `LogRedactor`) as the single logging choke point; every future backend feature ([session](session.md), [analytics](analytics.md), [mfa-otp](mfa-otp.md)) flows events through `AppLogger`, so these regexes automatically protect every later feature.
