...
- **Partly present.** This is a deepening of an existing, tested file — not a new subsystem. The cross-cutting port map in [`decisions.md`](../decisions.md#d4--port-reuse-do-not-multiply-backends) already names `LogRedactor` as the single log choke point; every future backend feature ([session](session.md), [analytics](analytics.md), [mfa-otp](mfa-otp.md)) flows events through it, so these regexes automatically protect every later feature.
